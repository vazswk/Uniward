function S_STRUCT = J_UNIWARD_3(coverPath, payload)
...
dct_coef_cover = double(C_COEFFS);
deta_q = (rand(size(dct_coef_cover)) - 0.5) * 5;
jpg_dct = dequantize(dct_coef_cover,C_QUANT);
jpg_dct = jpg_dct + deta_q;
C_SPATIAL = (ibdct(jpg_dct)+128);
...
for row = 1:k
    for col = 1:l
        ...
        subRows = row-modRow+(padSize-7+1):row-modRow+(8+8+padSize); %滤波器的大小为16*16，修改一个dct系数会影响8*8的像素，即影响23*23的小波系数
        subCols = col-modCol+(padSize-7+1):col-modCol+(8+8+padSize);
