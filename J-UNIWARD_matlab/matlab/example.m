...
%% Settings
payload = 0.4;   % measured in bits per non zero AC coefficients
coverPath = fullfile('..','images_cover', '1013.jpg');
stegoPath = fullfile('..','images_stego', '1013.jpg');
...
em_dct_coef = S_STRUCT.coef_arrays{1};

D = double(dct_coef) - double(em_dct_coef);
for mod_m = 1:8
    for mod_n = 1:8
        D_mode(1+(512/8)*(mod_m-1):(512/8)*mod_m,1+(512/8)*(mod_n-1):(512/8)*mod_n) = D(mod_m:8:end,mod_n:8:end);    
    end
end
show_cost_dis(D_mode)
 ...
 sum(abs(D(:)))
[HF_N, MF_N,  LF_N ] = dct_statistic_emb_num(dct_coef,D)
